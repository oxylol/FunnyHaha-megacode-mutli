...
public sealed class NetworkPlayer : MonoBehaviour
{
    ...
    public void Update()
    {
        if (model == null)
        {
            model = playerManager.GetOrAddPlayer(ID);
            return;
        }
        ...
        if (!IsLocal)
        {
            var timer = Mathf.InverseLerp(interpolationStart, interpolationEnd, Time.unscaledTime);
        ...
        if (transformTimer < 0)
        {
            ...
            if (IsLocal)
            {
                Client.Managers.RemotePlayerManager.SendPlayerUpdate(
